...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_title">
				<h2>ოთახები</h2>
				<a href="{{url('admin/'.$strlowerModel.'/create?model='.$returningModel)}}" class="btn btn-sm btn-success pull-right">
						...
						@foreach($items as $item)
							<tr>
								...
								<td class="text-center">
									...
									<td class="text-left">
										<a href="{{url('admin/'. $strlowerModel .'/edit/'.$item->id.'?model='.$returningModel)}}" title="რედაქტირება">
											@php
											$names = $item->{$returningModel.'Detail'}()->where('lang', 'ge')->first();
											echo $names->title;
											@endphp
										</a>
									</td>
									...
									<td align="right">
										<a href="{{url('admin/'. $strlowerModel .'/edit/'.$item->id.'?model='.$returningModel)}}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> </a>
										<a href="{{url('admin/'. $strlowerModel .'/delete/'.$item->id.'?model='. $returningModel)}}" class="delete btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>  </a>
									</td>
	...
	$(document).ready(function() {
		...
		$('.publish').change(function(event) {
			...
			$.ajax({
				...
				url: '{{url("admin/". $strlowerModel ."/publish?model=".$returningModel)}}',
				type: "POST",
